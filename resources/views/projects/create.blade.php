...
@section('content')
    <div class="mt-8 dark:bg-gray-800 overflow-hidden max-w-4xl mx-auto">
        ...
        <div class="container bg-white shadow rounded p-5 py-20">
            <form action="{{ route('project.store') }}" method="POST" class="flex justify-between">
                @csrf
                <div class="w-3/4">
                    <label for="name" class="w-1/4">Project Name</label>
                    <input type="text" name="name" id="name" class="bg-gray-100 px-2 rounded border w-3/4" value="" required>
                    @error('name')
