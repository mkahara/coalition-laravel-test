...
@section('content')
    <div class="mt-8 dark:bg-gray-800 overflow-hidden max-w-4xl mx-auto">
        ...
        <div class="container bg-white shadow rounded p-5 py-10 sm:py-20">
            <form action="{{ route('project.update', $project) }}" method="POST" class="flex justify-between flex-wrap">
                @csrf
                @method('PUT')
                <div class="mb-4 flex">
                    <label for="name" class="mr-2">Project Name</label>
                    <input type="text" name="name" id="name" class="bg-gray-100 px-2 rounded border" value="{{ $project->name }}" required>
                    @error('name')
